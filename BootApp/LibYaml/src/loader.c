...
static int yaml_parser_set_composer_error_context(yaml_parser_t *parser, const char *context, yaml_mark_t context_mark,
...
yaml_parser_load(yaml_parser_t *parser, yaml_document_t *document)
{
    ...
    if (!parser->stream_start_produced)
    {
        if (!yaml_parser_parse(parser, &event))
            goto error;
        ASSERT_MACRO(event.type == YAML_STREAM_START_EVENT);
...
static int yaml_parser_load_document(yaml_parser_t *parser, yaml_event_t *event)
{
    ...
    ASSERT_MACRO(event->type == YAML_DOCUMENT_START_EVENT);
    ...
    do
    {
        ...
        switch (event.type)
        {
            ...
            case YAML_MAPPING_END_EVENT:
                ...
                break;
            default:
                ASSERT_MACRO(0); /* Could not happen. */
                return 0;
...
static int yaml_parser_register_anchor(yaml_parser_t *parser, int index, yaml_char_t *anchor)
{
    ...
    {
        if (StrCmpMacro((char *)alias_data->anchor, (char *)anchor) == 0)
        {
            yaml_free(anchor);
            return yaml_parser_set_composer_error_context(parser, "found duplicate anchor; first occurrence",
...
static int yaml_parser_load_node_add(yaml_parser_t *parser, struct loader_ctx *ctx, int index)
{
    ...
    switch (parent->type)
    {
        case YAML_SEQUENCE_NODE:
            if (!STACK_LIMIT(parser, parent->data.sequence.items, INT_MAX_MACRO - 1))
                return 0;
        ...
        case YAML_MAPPING_NODE:
        {
            ...
            pair.key   = index;
            pair.value = 0;
            if (!STACK_LIMIT(parser, parent->data.mapping.pairs, INT_MAX_MACRO - 1))
                return 0;
...
static int yaml_parser_load_scalar(yaml_parser_t *parser, yaml_event_t *event, struct loader_ctx *ctx)
{
    ...
    if (!STACK_LIMIT(parser, parser->document->nodes, INT_MAX_MACRO - 1))
        goto error;

    if (!tag || StrCmpMacro((char *)tag, "!") == 0)
    {
        yaml_free(tag);
    ...
    SCALAR_NODE_INIT(node, tag, event->data.scalar.value, event->data.scalar.length, event->data.scalar.style,
...
static int yaml_parser_load_sequence(yaml_parser_t *parser, yaml_event_t *event, struct loader_ctx *ctx)
{
    ...
    SEQUENCE_NODE_INIT(node, tag, items.start, items.end, event->data.sequence_start.style, event->start_mark,
                       event->end_mark);
    ...
    if (!STACK_LIMIT(parser, *ctx, INT_MAX_MACRO - 1))
        return 0;
...
static int yaml_parser_load_sequence_end(yaml_parser_t *parser, yaml_event_t *event, struct loader_ctx *ctx)
{
    ...
    ASSERT_MACRO(((*ctx).top - (*ctx).start) > 0);

    index = *((*ctx).top - 1);
    ASSERT_MACRO(parser->document->nodes.start[index - 1].type == YAML_SEQUENCE_NODE);
...
static int yaml_parser_load_mapping(yaml_parser_t *parser, yaml_event_t *event, struct loader_ctx *ctx)
{
    ...
    MAPPING_NODE_INIT(node, tag, pairs.start, pairs.end, event->data.mapping_start.style, event->start_mark,
                      event->end_mark);
...
static int yaml_parser_load_mapping_end(yaml_parser_t *parser, yaml_event_t *event, struct loader_ctx *ctx)
{
    ...
    index = *((*ctx).top - 1);
    ASSERT_MACRO(parser->document->nodes.start[index - 1].type == YAML_MAPPING_NODE);
    parser->document->nodes.start[index - 1].end_mark = event->end_mark;
