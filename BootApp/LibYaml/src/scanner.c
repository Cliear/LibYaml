...
#define SKIP(parser)                                                                                                   \
    (parser->mark.index++, parser->mark.column++, parser->unread--, parser->buffer.pointer += WIDTH(parser->buffer))

#define SKIP_LINE(parser)                                                                                              \
    (IS_CRLF(parser->buffer)    ? (parser->mark.index += 2, parser->mark.column = 0, parser->mark.line++,              \
                                parser->unread -= 2, parser->buffer.pointer += 2)                                   \
     : IS_BREAK(parser->buffer) ? (parser->mark.index++, parser->mark.column = 0, parser->mark.line++,                 \
...
#define READ(parser, string)                                                                                           \
    (STRING_EXTEND(parser, string)                                                                                     \
         ? (COPY(string, parser->buffer), parser->mark.index++, parser->mark.column++, parser->unread--, 1)            \
         : 0)
                 ...
                 : (CHECK_AT(parser->buffer, '\r', 0) || CHECK_AT(parser->buffer, '\n', 0))                            \
                       ...
                       : (CHECK_AT(parser->buffer, '\xC2', 0) && CHECK_AT(parser->buffer, '\x85', 1))                  \
                             ...
                             (*((string).pointer++) = (yaml_char_t)'\n', parser->buffer.pointer += 2,                  \
                              parser->mark.index++, parser->mark.column = 0, parser->mark.line++, parser->unread--)    \
...
static int yaml_parser_set_scanner_error(yaml_parser_t *parser, const char *context, yaml_mark_t context_mark,
                                         const char *problem);
...
static int yaml_parser_roll_indent(yaml_parser_t *parser, PTR_DIFF_T_MACRO column, PTR_DIFF_T_MACRO number,
                                   yaml_token_type_t type, yaml_mark_t mark);

static int yaml_parser_unroll_indent(yaml_parser_t *parser, PTR_DIFF_T_MACRO column);
...
static int yaml_parser_scan_version_directive_value(yaml_parser_t *parser, yaml_mark_t start_mark, int *major,
                                                    int *minor);

static int yaml_parser_scan_version_directive_number(yaml_parser_t *parser, yaml_mark_t start_mark, int *number);

static int yaml_parser_scan_tag_directive_value(yaml_parser_t *parser, yaml_mark_t mark, yaml_char_t **handle,
                                                yaml_char_t **prefix);
...
static int yaml_parser_scan_tag_handle(yaml_parser_t *parser, int directive, yaml_mark_t start_mark,
                                       yaml_char_t **handle);

static int yaml_parser_scan_tag_uri(yaml_parser_t *parser, int uri_char, int directive, yaml_char_t *head,
                                    yaml_mark_t start_mark, yaml_char_t **uri);

static int yaml_parser_scan_uri_escapes(yaml_parser_t *parser, int directive, yaml_mark_t start_mark,
                                        yaml_string_t *string);

static int yaml_parser_scan_block_scalar(yaml_parser_t *parser, yaml_token_t *token, int literal);

static int yaml_parser_scan_block_scalar_breaks(yaml_parser_t *parser, int *indent, yaml_string_t *breaks,
...
yaml_parser_scan(yaml_parser_t *parser, yaml_token_t *token)
{
    ...
    ASSERT_MACRO(token);  /* Non-NULL token object is expected. */
...
static int yaml_parser_fetch_next_token(yaml_parser_t *parser)
{
    ...
    if (parser->mark.column == 0 && CHECK_AT(parser->buffer, '-', 0) && CHECK_AT(parser->buffer, '-', 1) &&
        CHECK_AT(parser->buffer, '-', 2) && IS_BLANKZ_AT(parser->buffer, 3))
...
static int yaml_parser_stale_simple_keys(yaml_parser_t *parser)
{
    ...
    for (simple_key = parser->simple_keys.start; simple_key != parser->simple_keys.top; simple_key++)
    {
            ...
            (simple_key->mark.line < parser->mark.line || simple_key->mark.index + 1024 < parser->mark.index))
        {
            ...
            {
                return yaml_parser_set_scanner_error(parser, "while scanning a simple key", simple_key->mark,
...
static int yaml_parser_save_simple_key(yaml_parser_t *parser)
{
    ...
    int required = (!parser->flow_level && parser->indent == (PTR_DIFF_T_MACRO)parser->mark.column);
...
static int yaml_parser_increase_flow_level(yaml_parser_t *parser)
{
    ...
    if (parser->flow_level == INT_MAX_MACRO)
    {
...
static int yaml_parser_scan_to_next_token(yaml_parser_t *parser)
{
    ...
    while (1)
    {
        ...
        while (CHECK(parser->buffer, ' ') ||
               ((parser->flow_level || !parser->simple_key_allowed) && CHECK(parser->buffer, '\t')))
        {
            SKIP(parser);
            if (!CACHE(parser, 1))
                return 0;
...
int yaml_parser_scan_directive(yaml_parser_t *parser, yaml_token_t *token)
{
    ...
    if (StrCmpMacro((char *)name, "YAML") == 0)
    {
    ...
    else if (StrCmpMacro((char *)name, "TAG") == 0)
    {
                                                    ...
                                                    int *minor)
{
    ...
    if (!CHECK(parser->buffer, '.'))
    {
        return yaml_parser_set_scanner_error(parser, "while scanning a %YAML directive", start_mark,
...
static int yaml_parser_scan_tag_directive_value(yaml_parser_t *parser, yaml_mark_t start_mark, yaml_char_t **handle,
                                                yaml_char_t **prefix)
{
...
static int yaml_parser_scan_anchor(yaml_parser_t *parser, yaml_token_t *token, yaml_token_type_t type)
{
    ...
    if (!length || !(IS_BLANKZ(parser->buffer) || CHECK(parser->buffer, '?') || CHECK(parser->buffer, ':') ||
                     CHECK(parser->buffer, ',') || CHECK(parser->buffer, ']') || CHECK(parser->buffer, '}') ||
                     CHECK(parser->buffer, '%') || CHECK(parser->buffer, '@') || CHECK(parser->buffer, '`')))
    {
        yaml_parser_set_scanner_error(
            parser, type == YAML_ANCHOR_TOKEN ? "while scanning an anchor" : "while scanning an alias", start_mark,
...
static int yaml_parser_scan_tag(yaml_parser_t *parser, yaml_token_t *token)
{
    ...
    else
    {
        ...
        if (handle[0] == '!' && handle[1] != '\0' && handle[StrLenMacro((char *)handle) - 1] == '!')
        {
            /* Scan the suffix now. */
                                       ...
                                       yaml_char_t **handle)
{
    ...
    if (!CHECK(parser->buffer, '!'))
    {
        yaml_parser_set_scanner_error(parser, directive ? "while scanning a tag directive" : "while scanning a tag",
    ...
    else
    {
        ...
        {
            yaml_parser_set_scanner_error(parser, "while parsing a tag directive", start_mark,
...
static int yaml_parser_scan_tag_uri(yaml_parser_t *parser, int uri_char, int directive, yaml_char_t *head,
                                    yaml_mark_t start_mark, yaml_char_t **uri)
{
    size_t        length = head ? StrLenMacro((char *)head) : 0;
    yaml_string_t string = NULL_STRING;
    ...
    if (length > 1)
    {
        MemCpyMacro(string.start, head + 1, length - 1);
    ...
    if (!length)
    {
        ...
        yaml_parser_set_scanner_error(parser, directive ? "while parsing a %TAG directive" : "while parsing a tag",
        ...
        if (!width)
        {
            ...
            if (!width)
            {
                return yaml_parser_set_scanner_error(
                    parser, directive ? "while parsing a %TAG directive" : "while parsing a tag", start_mark,
...
static int yaml_parser_scan_flow_scalar(yaml_parser_t *parser, yaml_token_t *token, int single)
{
    ...
    while (1)
    {
        ...
        while (!IS_BLANKZ(parser->buffer))
        {
            ...
            else if (!single && CHECK(parser->buffer, '\\'))
            {
                ...
                {
                    case '0': *(string.pointer++) = '\0'; break;

                    case 'a': *(string.pointer++) = '\x07'; break;

                    case 'b': *(string.pointer++) = '\x08'; break;

                    case 't':
                    case '\t': *(string.pointer++) = '\x09'; break;

                    case 'n': *(string.pointer++) = '\x0A'; break;

                    case 'v': *(string.pointer++) = '\x0B'; break;

                    case 'f': *(string.pointer++) = '\x0C'; break;

                    case 'r': *(string.pointer++) = '\x0D'; break;

                    case 'e': *(string.pointer++) = '\x1B'; break;

                    case ' ': *(string.pointer++) = '\x20'; break;

                    case '"': *(string.pointer++) = '"'; break;
                    ...
                    case 'x': code_length = 2; break;

                    case 'u': code_length = 4; break;

                    case 'U': code_length = 8; break;
    ...
    SCALAR_TOKEN_INIT(*token, string.start, string.pointer - string.start,
                      single ? YAML_SINGLE_QUOTED_SCALAR_STYLE : YAML_DOUBLE_QUOTED_SCALAR_STYLE, start_mark, end_mark);
...
static int yaml_parser_scan_plain_scalar(yaml_parser_t *parser, yaml_token_t *token)
{
    ...
    SCALAR_TOKEN_INIT(*token, string.start, string.pointer - string.start, YAML_PLAIN_SCALAR_STYLE, start_mark,
                      end_mark);
