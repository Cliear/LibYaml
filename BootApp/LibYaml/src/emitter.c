...
#define PUT_BREAK(emitter)                                                                                             \
    (FLUSH(emitter) && ((emitter->line_break == YAML_CR_BREAK     ? (*(emitter->buffer.pointer++) = (yaml_char_t)'\r') \
                         : emitter->line_break == YAML_LN_BREAK   ? (*(emitter->buffer.pointer++) = (yaml_char_t)'\n') \
                         : emitter->line_break == YAML_CRLN_BREAK ? (*(emitter->buffer.pointer++) = (yaml_char_t)'\r', \
...
static int yaml_emitter_emit_node(yaml_emitter_t *emitter, yaml_event_t *event, int root, int sequence, int mapping,
                                  int simple_key);
...
static int yaml_emitter_write_indicator(yaml_emitter_t *emitter, const char *indicator, int need_whitespace,
...
static int yaml_emitter_write_tag_content(yaml_emitter_t *emitter, yaml_char_t *value, size_t length,
                                          int need_whitespace);

static int yaml_emitter_write_plain_scalar(yaml_emitter_t *emitter, yaml_char_t *value, size_t length,
                                           int allow_breaks);

static int yaml_emitter_write_single_quoted_scalar(yaml_emitter_t *emitter, yaml_char_t *value, size_t length,
                                                   int allow_breaks);

static int yaml_emitter_write_double_quoted_scalar(yaml_emitter_t *emitter, yaml_char_t *value, size_t length,
                                                   int allow_breaks);
...
static int yaml_emitter_need_more_events(yaml_emitter_t *emitter)
{
    ...
    {
        case YAML_DOCUMENT_START_EVENT: accumulate = 1; break;
        case YAML_SEQUENCE_START_EVENT: accumulate = 2; break;
        case YAML_MAPPING_START_EVENT: accumulate = 3; break;
        default: return 0;
    }
...
static int yaml_emitter_append_tag_directive(yaml_emitter_t *emitter, yaml_tag_directive_t value, int allow_duplicates)
{
    ...
    {
        if (StrCmpMacro((char *)value.handle, (char *)tag_directive->handle) == 0)
        {
            if (allow_duplicates)
                return 1;
...
static int yaml_emitter_state_machine(yaml_emitter_t *emitter, yaml_event_t *event)
{
    ...
    {
        case YAML_EMIT_STREAM_START_STATE: return yaml_emitter_emit_stream_start(emitter, event);

        case YAML_EMIT_FIRST_DOCUMENT_START_STATE: return yaml_emitter_emit_document_start(emitter, event, 1);

        case YAML_EMIT_DOCUMENT_START_STATE: return yaml_emitter_emit_document_start(emitter, event, 0);

        case YAML_EMIT_DOCUMENT_CONTENT_STATE: return yaml_emitter_emit_document_content(emitter, event);

        case YAML_EMIT_DOCUMENT_END_STATE: return yaml_emitter_emit_document_end(emitter, event);

        case YAML_EMIT_FLOW_SEQUENCE_FIRST_ITEM_STATE: return yaml_emitter_emit_flow_sequence_item(emitter, event, 1);

        case YAML_EMIT_FLOW_SEQUENCE_ITEM_STATE: return yaml_emitter_emit_flow_sequence_item(emitter, event, 0);

        case YAML_EMIT_FLOW_MAPPING_FIRST_KEY_STATE: return yaml_emitter_emit_flow_mapping_key(emitter, event, 1);

        case YAML_EMIT_FLOW_MAPPING_KEY_STATE: return yaml_emitter_emit_flow_mapping_key(emitter, event, 0);

        case YAML_EMIT_FLOW_MAPPING_SIMPLE_VALUE_STATE: return yaml_emitter_emit_flow_mapping_value(emitter, event, 1);

        case YAML_EMIT_FLOW_MAPPING_VALUE_STATE: return yaml_emitter_emit_flow_mapping_value(emitter, event, 0);

        case YAML_EMIT_BLOCK_SEQUENCE_FIRST_ITEM_STATE: return yaml_emitter_emit_block_sequence_item(emitter, event, 1);

        case YAML_EMIT_BLOCK_SEQUENCE_ITEM_STATE: return yaml_emitter_emit_block_sequence_item(emitter, event, 0);

        case YAML_EMIT_BLOCK_MAPPING_FIRST_KEY_STATE: return yaml_emitter_emit_block_mapping_key(emitter, event, 1);

        case YAML_EMIT_BLOCK_MAPPING_KEY_STATE: return yaml_emitter_emit_block_mapping_key(emitter, event, 0);

        case YAML_EMIT_BLOCK_MAPPING_SIMPLE_VALUE_STATE:
            return yaml_emitter_emit_block_mapping_value(emitter, event, 1);

        case YAML_EMIT_BLOCK_MAPPING_VALUE_STATE: return yaml_emitter_emit_block_mapping_value(emitter, event, 0);

        case YAML_EMIT_END_STATE: return yaml_emitter_set_emitter_error(emitter, "expected nothing after STREAM-END");

        default: ASSERT_MACRO(1); /* Invalid state. */
    }
...
static int yaml_emitter_emit_stream_start(yaml_emitter_t *emitter, yaml_event_t *event)
{
    ...
    if (event->type == YAML_STREAM_START_EVENT)
    {
        ...
        {
            emitter->best_width = INT_MAX_MACRO;
        }
        ...
        if (event->data.document_start.tag_directives.start != event->data.document_start.tag_directives.end)
        {
                 ...
                 tag_directive != event->data.document_start.tag_directives.end; tag_directive++)
            {
                ...
                if (!yaml_emitter_write_tag_handle(emitter, tag_directive->handle,
                                                   StrLenMacro((char *)tag_directive->handle)))
                    return 0;
                if (!yaml_emitter_write_tag_content(emitter, tag_directive->prefix,
                                                    StrLenMacro((char *)tag_directive->prefix), 1))
                    return 0;
                if (!yaml_emitter_write_indent(emitter))
                    return 0;
            }
                                  ...
                                  int simple_key)
{
    ...
    switch (event->type)
    {
        case YAML_ALIAS_EVENT: return yaml_emitter_emit_alias(emitter, event);

        case YAML_SCALAR_EVENT: return yaml_emitter_emit_scalar(emitter, event);

        case YAML_SEQUENCE_START_EVENT: return yaml_emitter_emit_sequence_start(emitter, event);

        case YAML_MAPPING_START_EVENT: return yaml_emitter_emit_mapping_start(emitter, event);
...
static int yaml_emitter_check_simple_key(yaml_emitter_t *emitter)
{
    ...
    switch (event->type)
    {
        case YAML_ALIAS_EVENT: length += emitter->anchor_data.anchor_length; break;
        ...
        case YAML_SEQUENCE_START_EVENT:
            if (!yaml_emitter_check_empty_sequence(emitter))
                return 0;
            length +=
                emitter->anchor_data.anchor_length + emitter->tag_data.handle_length + emitter->tag_data.suffix_length;
            break;
...
static int yaml_emitter_process_scalar(yaml_emitter_t *emitter)
{
    switch (emitter->scalar_data.style)
    {
        case YAML_PLAIN_SCALAR_STYLE:
            return yaml_emitter_write_plain_scalar(emitter, emitter->scalar_data.value, emitter->scalar_data.length,
        ...
        default: ASSERT_MACRO(1); /* Impossible. */
    }
...
static int yaml_emitter_analyze_tag_directive(yaml_emitter_t *emitter, yaml_tag_directive_t tag_directive)
{
    ...
    handle_length = StrLenMacro((char *)tag_directive.handle);
    prefix_length = StrLenMacro((char *)tag_directive.prefix);
...
static int yaml_emitter_analyze_anchor(yaml_emitter_t *emitter, yaml_char_t *anchor, int alias)
{
    ...
    anchor_length = StrLenMacro((char *)anchor);
    STRING_ASSIGN(string, anchor, anchor_length);

    if (string.start == string.end)
    {
        return yaml_emitter_set_emitter_error(emitter, alias ? "alias value must not be empty"
...
static int yaml_emitter_analyze_tag(yaml_emitter_t *emitter, yaml_char_t *tag)
{
    ...
    tag_length = StrLenMacro((char *)tag);
    ...
    for (tag_directive = emitter->tag_directives.start; tag_directive != emitter->tag_directives.top; tag_directive++)
    {
        size_t prefix_length = StrLenMacro((char *)tag_directive->prefix);
        if (prefix_length < (size_t)(string.end - string.start) &&
            StrNCmpMacro((char *)tag_directive->prefix, (char *)string.start, prefix_length) == 0)
        {
            emitter->tag_data.handle        = tag_directive->handle;
            emitter->tag_data.handle_length = StrLenMacro((char *)tag_directive->handle);
...
static int yaml_emitter_analyze_scalar(yaml_emitter_t *emitter, yaml_char_t *value, size_t length)
{
    ...
    while (string.pointer != string.end)
    {
        ...
        {
            if (CHECK(string, '#') || CHECK(string, ',') || CHECK(string, '[') || CHECK(string, ']') ||
    ...
    switch (event->type)
    {
        ...
        case YAML_SCALAR_EVENT:
            ...
            if (event->data.scalar.tag &&
                (emitter->canonical || (!event->data.scalar.plain_implicit && !event->data.scalar.quoted_implicit)))
            {
                                        ...
                                        int is_whitespace, int is_indention)
{
    ...
    indicator_length = StrLenMacro(indicator);
                                          ...
                                          int need_whitespace)
{
    ...
    while (string.pointer != string.end)
    {
        if (IS_ALPHA(string) || CHECK(string, ';') || CHECK(string, '/') || CHECK(string, '?') || CHECK(string, ':') ||
            CHECK(string, '@') || CHECK(string, '&') || CHECK(string, '=') || CHECK(string, '+') ||
            CHECK(string, '$') || CHECK(string, ',') || CHECK(string, '_') || CHECK(string, '.') ||
            CHECK(string, '~') || CHECK(string, '*') || CHECK(string, '\'') || CHECK(string, '(') ||
        ...
        else
        {
            ...
            while (width--)
            {
                string_value = *(string.pointer++);
                if (!PUT(emitter, '%'))
                    return 0;
                if (!PUT(emitter, (string_value >> 4) + ((string_value >> 4) < 10 ? '0' : 'A' - 10)))
                    return 0;
                if (!PUT(emitter, (string_value & 0x0F) + ((string_value & 0x0F) < 10 ? '0' : 'A' - 10)))
                    return 0;
            }
        }
    }
                                                   ...
                                                   int allow_breaks)
{
    ...
    while (string.pointer != string.end)
    {
        if (!IS_PRINTABLE(string) || (!emitter->unicode && !IS_ASCII(string)) || IS_BOM(string) || IS_BREAK(string) ||
            CHECK(string, '"') || CHECK(string, '\\'))
        {
            unsigned char octet;
            unsigned int  width;
            ...
            width = (octet & 0x80) == 0x00   ? 1
                    ...
                    : (octet & 0xF8) == 0xF0 ? 4
                                             : 0;
            string_value = (octet & 0x80) == 0x00   ? octet & 0x7F
            ...
            for (k = 1; k < (int)width; k++)
            {
                ...
                string_value = (string_value << 6) + (octet & 0x3F);
            }
            ...
            switch (string_value)
            {
                ...
                default:
                    if (string_value <= 0xFF)
                    {
                        if (!PUT(emitter, 'x'))
                            return 0;
                        width = 2;
                    }
                    else if (string_value <= 0xFFFF)
                    {
                        if (!PUT(emitter, 'u'))
                            return 0;
                        width = 4;
